...
range_: int                             = 1
morning_hour: int                       = 7
evening_hour: int                       = 24# if not scraper.give_me_economics() else 20
...
position_size: float                    = round((100/len(symbols))*(0.9/divider()), 3)  # percent of balance 0.24 for tickmill europe broker leverage 1:30
kill_multiplier: float                  = 1.5  # loss of daily volatility by one position multiplier
tp_miner: int                           = 2
master_interval: str                    = intervals[0]
trigger_model_divider_factor: int       = 15
