...
def stoch_divergence_strategy_M1(df_raw, slow, fast):
    df = df_raw.copy()
...
def cci_divergence_strategy_M1(df_raw, slow, fast):
    df = df_raw.copy()
...
def moving_averages_close_M1(df_raw, slow, fast):
    df = df_raw.copy()
    df['adj'] = (df['close'] + df['high'] + df['low']) / 3
    ma1 = df.ta.vwma(length=fast)
    ma2 = ta.vwma(df['adj'], df['volume'], length=slow)
...
def macd_signal_M1(df_raw, slow, fast):
    df = df_raw.copy()
...
def macd_divergence_strategy_M1(df_raw, slow, fast):
    df = df_raw.copy()
